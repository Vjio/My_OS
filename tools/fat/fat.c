...
bool readRootDirectory(FILE* disk) {
	...
	// round up as to not leave out any data
...
bool readFile(DirectoryEntry* fileEntry, FILE* disk, uint8_t* outputBuffer) {
    ...
    do {
        ...
        ok = ok && readSectors(disk, lba, bootSector.SectorsPerCluster, outputBuffer);
...
// a FAT structure looks like so: RESERVED, actual FAT, then data directories
